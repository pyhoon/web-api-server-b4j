...
Private Sub ProcessRequest
	Try
		Select Request.Method.ToUpperCase
			...
			Case "POST"
				Select ElementLastIndex
					Case SECOND_ELEMENT ' /password/:action
						If Elements(FIRST_ELEMENT) = Literals(1) Then
							Select Elements(SECOND_ELEMENT)
								Case "change"
...
Private Sub ReturnResponse (Message As HttpResponseMessage)
	If Main.SimpleResponse Then
		Utility.ReturnSimpleHttpResponse(Message, Main.SimpleResponseFormat, Response)
	Else
	...
	Try
		...
		Select action
			...
			Case "reset"
				...
				<br />
				<a href="${Main.ROOT_URL}${Main.ROOT_PATH}password/confirm-reset/${reset_code}" id="reset-link" title="reset" target="_blank">${Main.ROOT_URL}${Main.ROOT_PATH}password/confirmreset/${reset_code}</a><br />
				<br />
