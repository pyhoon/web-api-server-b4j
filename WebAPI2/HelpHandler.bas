...
@EndOfDesignText@
' Help Handler class
' Version 2.00 revised for beta 3
Sub Class_Globals
...
Public Sub ReadControllers (FileDir As String) As String
	...
	For Each Controller As String In DocumentedControllers
		...
		For Each m As Map In Methods
			...
			If m.ContainsKey("Controller") = False Then m.Put("Controller", ControllerName.ToLowerCase)			
			strHtml = strHtml & GenerateDocItem(m.Get("Version"), m.Get("Controller"), m.Get("Elements"), m.Get("Verb"), MethodName, m.Get("Prm"), m.Get("Body"), m.Get("Format"), m.Get("Desc"))
		Next
...
Public Sub GenerateDocItem (ApiVersion As String, Controller As String, Elements As List, Verb As String, MethodName As String, Params As List, Body As String, DefaultFormat As String, Desc As String) As String
	Dim strHTML As String
