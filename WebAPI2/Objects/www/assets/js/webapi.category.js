$(document).ready(function () {
  $.getJSON("/web/category/sort/id", function (response) {
    var tbl_head = "";
    var tbl_body = "";
    if (response.r.length) {
      tbl_head = "<thead class=\"bg-light\"><th style=\"text-align: right; width: 60px\">ID</th><th>Name</th><th style=\"width: 90px\">Actions</th></thead>";
      tbl_body += "<tbody>";
  ...
  form.validate({
    ...
    submitHandler: function (form) {
      ...
      $.ajax({
        ...
        dataType: "json",
        type: "put",
        url: "/web/api/v2/category/" + $('#id1').val(),
        success: function (response) {
          $('#edit').modal('hide');
          if (response.a == 200) {
...
$(document).on('click', '#remove', function (e) {
  ...
  $.ajax({
    ...
    dataType: "json",
    type: "delete",
    url: "/web/api/v2/category/" + $('#id2').val(),
    success: function (response) {
      $('#delete').modal('hide');
      if (response.a == 200) {
